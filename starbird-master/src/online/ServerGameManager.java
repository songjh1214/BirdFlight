...
public class ServerGameManager {
	...
	public void checkCollision(){//Ãæµ¹È®ÀÎ??
		checkBulletCollision(); //ÃÑ¾ËÃæµ¿
		checkMobCollision(); //¸÷Ãæµ¿
		checkDropCollision();
	...
	private void updateStatus(){ 
		...
		for (Player p : game.players){
			if (p.health > 0) aliveflag = true; //ÇÃ·¹ÀÌ¾î°¡ »ì¾ÆÀÕÀ»¶§ alive flag true ¹«½¼¸»ÀÎÁö ¸ð¸£°ÙÀ½.
		}
	...
	public void performUpdateStatus(){ //¾÷µ¥ÀÌÆ® ÁøÇàÇÏ´Â »óÅÂ??
	...
	public void performCollisionStatus(){ // Ãæµ¹ ÁøÇàµÇ´Â »óÅÂ
		if (game.gameStatus == 1){
			// ½ÃÀÛÀ» ±â´Ù¸²
		}
		// °ÔÀÓ ÁøÇàµÉ¶§
	...
	public void updatePlayer(Player p){ //ÇÃ·¹ÀÌ¾îÃß°¡
		if(!existingPlayer(p)){
			game.addPlayer(p);
		}
	...
	private void updateBots(){ //º¿ ¾÷µ¥ÀÌÆ®
		for (Birdbot b : game.bots){
			b.update();
		}
	}

	private void updateEnemies(){ //Àû À¯´Ö ¾÷µ¥ÀÌÆ®
		for (EnemyUnit b : game.birds){
			b.update();
		}
	}

	private void updateObjects(){ //ÄÚÀÎ ¾÷µ¥ÀÌÆ®
		for (CoinDrop d: game.drops){
			d.update();
		}
	}

	private void checkBulletCollision(){ //È÷Æ®¹Ú½º¿¡ ¸Â­ŸÀ»¶§ ÃÑ¾Ë Ãæµ¹.
		for (EnemyUnit e : game.birds){
			...
			if(e.isDead()) { //Á×¾ùÀ»¶§
				...
				game.score += e.scoreValue; //½ºÄÚ¾î ´©ÀûµÈ´Ù´ÂÀÇ¹Ì°°À½.
			}
		}
	}

	private void checkMobCollision(){ //¸÷Ãæµ¹
	...
	private void cleanDeadObjects(){ //Á×Àº ¿ÀºêÀèÆ® ¸Ê¿¡¼­ Á¦°Å.
		// Á×Àº ¸÷µé Á¦°Å
		ArrayList<EnemyUnit> enemyRemoveList = new ArrayList<EnemyUnit>();
		for (EnemyUnit e : game.birds){
	...
	private double botGetDistance(Birdbot bot, EnemyUnit en){
		int xposD = Math.abs(bot.xpos - en.xpos); //Math.abs ÇÔ¼ö´Â ÁÖ¾îÁø ¼ýÀÚÀÇ Àý´ë°ªÀ» ¹ÝÈ¯ ¹ºÀÇ¹ÌÀÎÁö ¸ð¸£°ÚÀ½.
	...
	private boolean checkIfTargeted(EnemyUnit enemy){ //ÀûÅ¸°ÙÀ» Ã¼Å©ÇØÁÖ´Â.
	...
	private EnemyUnit botGetTarget(Birdbot bot){ //´Ù½ÃÃ¼Å©
	...
	private void updateTargets(){ //Å¸ÄÏ¾÷µ¥ÀÌÆ®
		for (Birdbot b : game.bots){
